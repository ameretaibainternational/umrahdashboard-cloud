...
import { upsertTransportContact, deleteTransportContact } from '@/app/actions/settings'
import type { TransportContact } from '@/lib/types'
...
interface Props { contacts: TransportContact[] }

const empty: Partial<TransportContact> = { name: '', city: 'Makkah', contact_number: '' }

export default function TransportContactsForm({ contacts }: Props) {
  const router = useRouter()
  const [editing, setEditing] = useState<Partial<TransportContact> | null>(null)
  const [editCity, setEditCity] = useState('Makkah')
  const [deleteId, setDeleteId] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()

  function openEditor(contact: Partial<TransportContact>) {
  ...
  function handleUpsert(formData: FormData) {
    startTransition(async () => {
      const result = await upsertTransportContact(formData)
  ...
  function handleDelete() {
    if (!deleteId) return
    startTransition(async () => {
      const result = await deleteTransportContact(deleteId)
      if ('error' in result && result.error) {
        toast.error(result.error)
        return
      }
    ...
    <>
      <Card className="shadow-sm border-0 max-w-3xl">
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-base">Transport Contacts</CardTitle>
            <p className="text-xs text-muted-foreground mt-1">
              Used on transport vouchers — select by provider name and city; only the contact number prints on the voucher.
            </p>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40">
                <TableHead className="text-xs">Transport Provider Name</TableHead>
              ...
              {contacts.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center text-muted-foreground py-8 text-sm">
                    No transport contacts yet — click Add Contact to create one.
                  </TableCell>
                </TableRow>
              ) : contacts.map(c => (
      ...
      <Dialog open={!!editing} onOpenChange={open => !open && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing?.id ? 'Edit Transport Contact' : 'Add Transport Contact'}</DialogTitle>
          </DialogHeader>
          <form action={handleUpsert} className="space-y-4">
            {editing?.id && <input type="hidden" name="id" value={editing.id} />}
            <input type="hidden" name="city" value={editCity} />
            <div className="space-y-1.5">
              <Label className="text-xs">Transport Provider Name *</Label>
              <Input name="name" defaultValue={editing?.name ?? ''} required placeholder="e.g. Makkah Transport Co" />
            </div>
            <div className="space-y-1.5">
      ...
      <Dialog open={!!deleteId} onOpenChange={open => !open && setDeleteId(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete Transport Contact</DialogTitle></DialogHeader>
