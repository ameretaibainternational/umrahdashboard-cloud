...
interface Props {
  ...
  existingInvoices: Pick<CustomInvoice, 'invoice_number'>[]
...
export default function CustomInvoiceForm({
  ...
  const resetForNewInvoice = useCallback((
    invoices: Pick<CustomInvoice, 'invoice_number'>[],
  ...
  const handleSaveAndDownload = useCallback(async () => {
    ...
    try {
      ...
      if (profit > inv.total) {
        toast.error('Profit cannot exceed invoice total.')
        return
      }
      if (saveCustomer && billedName.trim()) {
        const clientForm = new FormData()
        clientForm.set('name', billedName.trim())
        clientForm.set('address', billedAddr.trim())
        clientForm.set('client_number', billedPhone.trim())
        const existingClient = savedClients.find(c => c.name === billedName.trim())
      ...
      if (updateId) {
        toast.success(`Invoice ${result.invoice_number} updated and downloaded.`)
        router.push('/invoices?tab=custom')
      } else {
              ...
              <div className="flex-1 min-w-0 max-w-[580px] space-y-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2">
                    Profit is counted in dashboard and accounts. The remainder (total − profit) is recorded as an expense automatically.
                  </p>
                </div>
