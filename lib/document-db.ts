  ...
  try {
    const [created] = await sql<{ id: string; invoice_number: string }[]>`
      INSERT INTO custom_invoices (
        ...
        storage_key, file_size_bytes, invoice_title_text, created_by, package_data, invoice_kind
      ) VALUES (
        ...
        ${baseValues.storage_key}, ${baseValues.file_size_bytes}, ${row.invoice_title_text ?? 'INVOICE'}, ${row.created_by ?? null}, ${baseValues.package_data}, 'custom'
      )
    ...
    if (message.includes('invoice_kind')) {
      const [created] = await sql<{ id: string; invoice_number: string }[]>`
        INSERT INTO custom_invoices (
          id, invoice_number, invoice_date, billed_to_name, billed_to_address, billed_to_client_number,
          payment_bank_name, payment_account_number, terms_text,
    ...
    if (message.includes('profit_pkr')) {
      const [created] = await sql<{ id: string; invoice_number: string }[]>`
        INSERT INTO custom_invoices (
          id, invoice_number, invoice_date, billed_to_name, billed_to_address, billed_to_client_number,
          payment_bank_name, payment_account_number, terms_text,
...
export function mapCustomInvoiceRow(r: Record<string, unknown>): CustomInvoice {
  ...
  return {
    ...
    profit_pkr: r.profit_pkr != null ? Number(r.profit_pkr) : 0,
...
export async function updateHotelVoucherDirect(
  id: string,
  row: {
    voucher_date: string
    reference_no: string
  ...
  const [existing] = await sql<{ storage_key: string | null }[]>`
    SELECT storage_key FROM hotel_vouchers WHERE id = ${id}
  `
  ...
  await sql`
    UPDATE hotel_vouchers SET
      ...
      voucher_data = ${sql.json(row.voucher_data as postgres.JSONValue)},
      storage_key = ${row.storage_key},
      file_size_bytes = ${row.file_size_bytes}
    WHERE id = ${id}
  `
...
export async function deleteHotelVoucherDirect(id: string, options?: { force?: boolean }) {
  const sql = requireWriteSql(options)
  const [v] = await sql<{ storage_key: string | null }[]>`
    SELECT storage_key FROM hotel_vouchers WHERE id = ${id}
  `
  if (v?.storage_key) {
    const { deletePdfKeys } = await import('@/lib/r2')
  ...
  await sql`DELETE FROM hotel_vouchers WHERE id = ${id}`
}

export async function updateHotelVoucher(
  id: string,
  row: {
    voucher_date: string
    reference_no: string
  ...
  return withDocumentDbFallback(
    () => updateHotelVoucherDirect(id, row),
    async () => {
      const { updateHotelVoucherSupabase } = await import('@/lib/supabase-document-db')
      return updateHotelVoucherSupabase(id, row)
    }
  )
}
...
export async function deleteHotelVouchers(ids: string[]) {
  return withDocumentDbFallback(
    async () => {
      const sql = requireWriteSql()
      await sql`DELETE FROM hotel_vouchers WHERE id = ANY(${ids})`
    },
    async () => {
      const supabase = await import('@/lib/supabase-document-db')
      for (const id of ids) {
        await supabase.deleteHotelVoucherSupabase(id)
      }
    }
  )
}
