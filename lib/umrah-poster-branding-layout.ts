...
export const POSTER_CORNER_SRC = '/poster-corner.svg'
export const DEFAULT_POSTER_CORNER_COLOR = '#1b376e'
...
export async function loadPosterCorner(color = DEFAULT_POSTER_CORNER_COLOR): Promise<HTMLImageElement> {
  try {
    const response = await fetch(POSTER_CORNER_SRC)
    if (!response.ok) throw new Error('Failed to fetch SVG corner')
    let svgText = await response.text()
    svgText = svgText.replace('#2d2e83', color)
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        URL.revokeObjectURL(url)
        reject(new Error('Failed to load SVG corner'))
      }
      img.src = url
    })
  } catch (err) {
