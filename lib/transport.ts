...
export function normalizeTransportType(value: unknown): TransportVehicle {
  if (typeof value === 'string' && isTransportVehicle(value)) return value
  if (value === 'private') return 'CAR'
  if (value === 'bus') return 'HIACE'
  return DEFAULT_TRANSPORT_VEHICLE
}

export function transportServiceName(vehicle: TransportVehicle): string {
  return `Transport — ${vehicle}`
}

export function transportServiceNames(): string[] {
  return TRANSPORT_VEHICLES.map(transportServiceName)
}
